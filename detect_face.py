...
#detector = cv2.CascadeClassifier("haarcascade_frontalface_default.xml"): dò tìm khuôn mặt Bộ dò tìm khuôn mặt sử dụng thuật toán Haar Cascade.
#Mô hình CNN (lenet.hdf5) được sử dụng để phân loại khuôn mặt.
#faces = detector.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30), flags=cv2.CASCADE_SCALE_IMAGE)
#

#cv2.CascadeClassifier()	Dò tìm khuôn mặt trong ảnh hoặc video.
#load_model()	Tải mô hình CNN để phân loại khuôn mặt.
#detectMultiScale()	Phát hiện khuôn mặt từ ảnh xám.
#predict()	Dự đoán nhãn khuôn mặt từ mô hình CNN.
#cv2.putText()	Hiển thị thông tin người dùng trên khung hình.
#xlwrite.output()	Lưu thông tin điểm danh vào file Excel.
